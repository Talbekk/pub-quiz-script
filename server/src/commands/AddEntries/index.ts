...
const createEntry = (participantId: Participant['_id'], messages: Array<Message>): Entry => {
...
const AddEntries = async () => {
    ...
    try {
        ...
        const processQuizBatch = async (quizzes: Array<Quiz>, messages: Array<Message>) => {
            const quizIds: Array<Quiz['_id']> = quizzes.map(quiz => quiz._id);
            ...
            const updatedQuizzes: Array<Quiz> = quizzes.map((quiz) => {
                ...
                const quizEntries = participants.map((participant) => createEntry(participant._id, messagesDuringQuizWeek));
