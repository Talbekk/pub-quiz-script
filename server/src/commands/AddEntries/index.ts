...
const createEntry = (
    ...
    const participantMessages: Array<Message> = messages.filter(
        (message) => message.participant_ref === participantId,
    );
    return {
        _id: ulid(),
...
const AddEntries = async () => {
    ...
    try {
        ...
        const quizzesCursor = quizzesCollection.find(
            {},
            { sort: [['_id', 1]] },
        );
        ...
        const processQuizBatch = async (
            ...
            const updatedQuizzes: Array<Quiz> = quizzes.map((quiz) => {
                const messagesDuringQuizWeek = messages.filter(
                    (message) =>
                        message.timestamp >= quiz.start_datetime &&
                        message.timestamp < quiz.end_datetime,
                );
                const quizEntries = participants.map((participant) =>
                    createEntry(
                        participant._id,
                        quiz._id,
                        messagesDuringQuizWeek,
                    ),
                );
        ...
        for await (const currentQuizzes of batchedStream) {
            ...
            const quizStartTimes = currentQuizzes.map(
                (quiz) => quiz.start_datetime,
            );
            const quizEndTimes = currentQuizzes.map(
                (quiz) => quiz.end_datetime,
            );
            const messagesDuringQuizBatch = await messagesCollection
                .find({
                    timestamp: {
                        $gte: Math.min(...quizStartTimes),
                        $lt: Math.max(...quizEndTimes),
                    },
                })
                .toArray();
