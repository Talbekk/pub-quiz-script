    ...
    try {
        ...
        const processQuizBatch = async (
            ...
            const updatedQuizzes: Array<Quiz> = quizzes.map((quiz) => {
                const quizEntries: Array<Entry> = entries.filter(
                    (entry) => entry.quiz_ref === quiz._id,
                );
                const quizMessages: Array<Message> = quizEntries.flatMap(
                    (entry) => entry.messages,
                );
                const quizLinkMessages: Array<LinkMessage> =
                    quizMessages.filter((message) => message.type === 'link');
                const quizUrls = quizLinkMessages
                    .map((quizLinkMessage) => quizLinkMessage.share.link)
                    .filter((url) => url !== undefined) as Array<string>;
                return {
                    ...quiz,
                    url: quizUrls,
                };
        ...
        for await (const currentQuizzes of batchedStream) {
            ...
            const entriesDuringQuizBatch = await entriesCollection
                .find({
