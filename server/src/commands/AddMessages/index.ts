...
export const AddMessages = async () => {
    ...
    try {
        ...
        const processMessageBatch = async (messages: Array<any>) => {
            const formattedMessages = messages.map((message) => {
                if(message.share) {
                    return {
                        ...
                        participant_ref: getParticipantRef(participants, message.sender_name),
                        share: message.share,
            ...
            const filteredMessages = formattedMessages.filter((message): message is Message => message !== undefined);
            try {
                const result = await messagesCollection.insertMany(filteredMessages, { ordered: true });
